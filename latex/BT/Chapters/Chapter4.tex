...
\begin{table}[!ht]
    ...
    $s_{t,d,x,y}$           &: &\begin{tabular}{l c l}
                                    \multicolumn{3}{l}{is true if and only if the cell at depth $d$ of}\\
                                \end{tabular}\\
                            &  &\begin{tabular}{l c l}
                                \multicolumn{3}{l}{thermometer number $t$ is located in grid cell}\\
                                \multicolumn{3}{l}{$(x,y).$}\\
                                 t      &$\in$  &$\{1,..,81\}$\\
                                 d,x,y      &$\in$  &$\{1,..,9\}$\\
                                 \multicolumn{3}{l}{\makecell[cl]{In practice $t$ has an offset of +10. So the actual\\
                                 variable range is: $11111$ to $91999$.}}\\
                            \end{tabular}\\
    \\
    \hline
\end{tabular}
...
\end{table}\todo{Fix Offset in variable section.}
...
\todoMissing{Description, Constraints, Formulae}

\FloatBarrier
\newpage
\subsection{Nurikabe Sudoku}
\todoMissing{Description Text}
...
\begin{table}
    ...
    $PBC_O(x,y,z)$          &= & Set of clauses needed to encode that in the direction where\\
                            ...
                            &  & of the ocean.\\
                            &  & LHS = $\begin{cases}
                                        \displaystyle\sum_{\substack{k=y-1\\--k}}^{1} 1*\ocean(x,k)       & (x,y) \textnormal{ contains}  \uparrow\\
                                        \\
                                        \displaystyle\sum_{\substack{k=y+1\\++k}}^{9} 1*\ocean(x,k)       & (x,y) \textnormal{ contains}  \downarrow\\
                                        \\
                                        \displaystyle\sum_{\substack{k=x-1\\--k}}^{1} 1*\ocean(k,y)       & (x,y) \textnormal{ contains}  \leftarrow\\
                                        \\
                                        \displaystyle\sum_{\substack{k=x+1\\++k}}^{9} 1*\ocean(k,y)       & (x,y) \textnormal{ contains}  \rightarrow\\
                                        \end{cases}$\\
                            &  & RHS = $z$\\
    \\
